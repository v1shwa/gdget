...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Command line utility to easily download Google Drive public URLs."
    )
    parser.add_argument(
        "--version",
        "-v",
        action="store_true",
        help="Display the version of gdget",
        required=False,
    )
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Turn off gdget's output", required=False
    )
    parser.add_argument("URL", help="Google drive URL or ID", nargs="?")
    parser.add_argument(
        "--output-document", "-O", help="Name of the output file", type=str, required=False
    )
